...
<script>
        ...
        layui.use(['form', 'layedit', 'laydate'], function(){
            ...
            form.verify({
                ...
                ,new_password: function(value){
                    ...
                    if(value.trim() == $("#old_password").val()){
    ...
    function resetPwd(data){
        ...
        $.post("/member/reset_pwd",params,function(result){
    ...
    function logout(){
        layer.confirm('{{ Lang.open_confirm_logout || "确认退出登录？" }}', {
            btn: ['{{ Lang.btn_confirm || "确认" }}','{{ Lang.btn_cancel || "取消" }}'] //按钮
        }, function(){
            $.post("/member/logout",{},function(result){
                if(result.status == 200){
                    delCookie("token")
                   delCookie("user")
    ...
    <div class="form" style="padding:20px;">
        ...
        <form class="layui-form" action="">
            <div class="layui-form-item">
                <label class="layui-form-label">{{ Lang.form_placeholder_old_password || "旧密码" }}</label>
                <div class="layui-input-block">
                    <input type="password" name="password" lay-verify="password" id="old_password" autocomplete="off"  class="layui-input">
                </div>
            </div>

            <div class="layui-form-item">
                <label class="layui-form-label">{{ Lang.form_placeholder_new_password || "新密码" }}</label>
                <div class="layui-input-block">
            ...
            <div class="layui-form-item">
                <label class="layui-form-label">{{ Lang.form_placeholder_confirm_new_password || "确认新密码" }}</label>
                <div class="layui-input-block">
            ...
            <div class="layui-form-item">
                <div class="layui-input-block">
                    <button class="layui-btn" lay-submit="" lay-filter="resetPwdSubmit" onclick="return false;">{{ Lang.btn_sub || "提交" || "提交" }}</button>
                </div>
            </div>
        </form>
