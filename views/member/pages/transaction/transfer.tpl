...
{% block js_assets %}
    ...
    <script>
        ...
        function submit(data) {
            ...
            submit_f = true;
            $.ajax({
                url: "/member/transaction/transfer/submit_transfer",
                async: true,
                data: data,
                type: "post",
                dataType: "json",
                ...
                success: function (data) {
                    ...
                    if (data.status == 200) {
                        layer.alert("{{ Lang.alert_transfer_submit_success1 || "您的转账请求已被系统接收，我们的工作人员将会尽快为您办理，请耐心等待。" }}<br/>" +
                            "<a href=\"/member/funds/transfer\">{{ Lang.alert_transfer_submit_success2 || "若您需要取消，请点击此处进行操作。" }}</a>", {
        ...
        function showConfirm(data) {
            ...
            layer.open({
                type:1,
                title:'{{ Lang.page_funds_transfer_verify_title || "转账订单确认" }}',
                area:['500px', 'auto'],
                content:$('#confirm')
            });
        }
...
{% block content %}
                    ...
                    <div class="form">
                        <div class="form_input">
                            <div class="from_alert">
                                <h3>{{ Lang.page_transaction_transfer_msg_h3 || "账户转账操作说明：" }}</h3>
                                <p>{{ Lang.page_transaction_transfer_msg_txt1 || "1. 在您进行转账操作前，请务必确保转出账户无交易风险（包括无未平仓订单、无挂单等），由于转账操作导致的任何损失， 平台不承担责任。" }}</p>
                                <p>{{ Lang.page_transaction_transfer_msg_txt2 || "2. 转账提交后，系统会尽快处理您的订单。" }}</p>
                            </div>

                            <div class="form">

                                <form class="layui-form w600">
                                    ...
                                    <div class="layui-form-item">
                                        <div class="layui-input-block">
                                            <button class="layui-btn" lay-submit lay-filter="formSubmit" onclick="return false;">{{ Lang.btn_sub || "提交" || "提交" }}</button>
    ...
    <div id="confirm" style="display:none">
        <div id="msg" style="margin: 20px 30px -16px 30px;">
            {{ Lang.form_text_info_true || " 您的转账信息如下所示，请确认信息无误后，点击“确定”按钮 " }}
        </div>
