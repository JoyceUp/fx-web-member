...
{% block js_assets %}
    <script>
        ...
        {% for at in Lang.select.member.funds.in_channel_type %}
        channelTypeMap[{{ at.value }}] = "{{ at.label }}";
        {% endfor %}

        //出金渠道类型
        {% for at in Lang.select.member.funds.out_channel_type %}
        channelTypeMap[{{ at.value }}] = "{{ at.label }}";
        {% endfor %}
        ...
        function showDetail(id){
            $.ajax({
                url:'/member/funds/transfer/info',
                async: true,
                type:'post',
                dataType:'json',
                data:{"id":id},
        ...
        function cancelTransfer(){
            ...
            var id = $("#id").val();
            $.ajax({
                url:'/member/transaction/transfer/cancel_transfer',
                async: true,
                type:'post',
                dataType:'json',
                data: {"id": id, "type":$("#channel_type").val()},
                contentType: "application/x-www-form-urlencoded;charset=utf-8",
                success:function(data){
                    if(data.status == 200){
        ...
        layui.use(['table','form','laydate'],function(){
            ...
            tableIns = table.render({
                elem:'#dataTable',
                url:'/member/funds/transfer/list',
                page:true,
                method:'post',
                cols:[[
                        ...
                        <form class="layui-form"  action="">
                            ...
                            <div class="layui-input-inline">
                                <select name="state">
                                    ...
                                    {% for at in Lang.select.member.funds.transfer_order_state %}
    ...
    <div id="detail" style="display:none">
        <form class="layui-form show-detail" action="">
            <div class="d-item">
                ...
                <input type="hidden" id="channel_type"/>
            </div>
