<div class="form">
    <form class="layui-form" action="">
        ...
        <div class="layui-form-item">
            ...
            <label class="layui-form-label"><i>*</i>{{ Lang.form_label_bank_city_ || "银行城市" }}:</label>
            <div class="layui-input-inline">
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="hidden" id="card_path" name="path" lay-verify="path" value=""/>
                <div class="layui-upload">
                    <button type="button" class="layui-btn" id="btn_upload" onclick="return false;">{{ Lang.btn_uploadImg || "上传图片" }}</button>
                    <div class="layui-upload-list">
                        <img class="layui-upload-img" id="img_upload" src="/assets/member/images/pic.png"/>
        ...
        <div class="layui-form-item">
            <div class="layui-input-block">
                <button class="layui-btn" lay-submit="" lay-filter="formSubmit" onclick="return false;">{{ Lang.btn_save || "保存" }}</button>
...
<div class="from_alert">
    ...
    <p>
        {{ Lang.page_user_bankinfo_add_msg_txt1 || "1.为保证正常出金，请填写准确的支行名称；若无支行，请填写分行名称。" }}
    </p>
    <p>
        {{ Lang.page_user_bankinfo_add_msg_txt2 || "2.本人理解提供银行资料的重要性，因为在本人的提款过程中，这些资料可能会被使用到。本人谨此声明，以上所提供的资料均属真实且正确。" }}
    </p>
</div>
...
<script>
    ...
    layui.use(['form', 'layedit', 'laydate'], function () {
        ...
        form.on('submit(formSubmit)', function (data) {
            ...
            var url = "/member/customer/bankinfo/add";
            var type = 0;
            if(data.field.id && data.field.id.length > 0){
                url = "/member/customer/bankinfo/update";
                type = 1;
            }
    ...
    function initUpload() {
        if (jQuery(".layui-upload-file").length > 0) {
            return;
        }
        var img_base64;
        layui.use('upload', function () {
            ...
            var uploadInst = upload.render({
                ...
                , done: function (res) {
                    ...
                    //上传成功
                    if (res.status === 200) {
                        jQuery("#card_path").val(JSON.parse(res.datas).id);
                        $('#img_upload').attr('src', img_base64); //图片链接（base64）
                    }
                }
                , error: function () {
                    //演示失败状态，并实现重传
    ...
    var bank_option =JSON.parse(jQuery.parseHTML("{{banks}}")[0].data);
    ...
    function initDialogData(data) {
        if (data && data.id.length > 0) {
            ...
            jQuery("#card_path").val(data.path);
            getImages(jQuery("#img_upload"), data.path);
        }
        else {
            emptyDialogData();
        }
    ...
    function emptyDialogData() {
        ...
        $("#card_city").val("");
        $("#img_upload").attr("src", "/assets/member/images/pic.png");
        $("#card_path").val("");
