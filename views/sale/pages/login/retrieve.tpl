...
<body>
	...
	<div class="layadmin-user-login-main">
		<div class="layadmin-user-login-box layadmin-user-login-header">
			<h2>{{ Lang.page_backpassword_title || "找回密码" }}</h2>
			<p></p>
		</div>
		<div class="layadmin-user-login-box layadmin-user-login-body layui-form">
			...
			<div class="layui-form-item mt30">
				<button class="layui-btn layui-btn-fluid" lay-submit="" id="sendEmailBut" lay-filter="user-login-submit">{{ Lang.btn_login_send_reset_password || "发送邮件, 重置密码" }}</button>
			</div>
		</form>
		</div>
	</div>
...
<script>
    ...
    function submit(data){
        $.post("/sale/password/retrieve",data,function(result){
