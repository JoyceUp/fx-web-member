...
{% block js_assets %}
    ...
    <script>
        ...
        var account_type_map = new Object();
        {% for at in Lang.select.ib.register.account_type %}
        account_type_map[{{ at.value }}] = '{{ at.label }}';
        {% endfor %}
        ...
        layui.use(['form'], function () {
                ...
                $.ajax({
                    ...
                    success: function (data) {
                        if (data.status == 200) {
                           ...
                           var accountType = JSON.parse(data.datas).items;
                           var optionItems = '';
                            if(accountType == 1){ // accountType = 1 上级销售是兼职，其下级只能开通标准账户
                               delete account_type_map[3];
                            }
                            for(var i in account_type_map){
                                optionItems +="<option value='"+i+"'>"+account_type_map[i]+"</option>";
                            }
                            $("#accountType").append(optionItems);
                            form.render('select');
                            return false;
                        } else {
            ...
            form.on('select(account_type)', function(data){
                ...
                //根据选择的账户类型，改变相应注册的协议链接

              if(data.value==3){

                  $("#agreement").attr("href","/assets/kh/file/cn/ib-ecn.pdf")
              }else{
                  $("#agreement").attr("href","/assets/kh/file/cn/BAPPEBTI.pdf")
              }
...
{% block content %}
    ...
    <div class="register" id="first_page">
        ...
        <div class="register-form">
            <p class="reg-title-1">{{ Lang.page_register_dl_tips_msg || "账户信息 " }}</p>
            <p class="reg-title-2">{{ Lang.page_register_dl_tips_msg || "账户信息 " }}<span>{{ Lang.page_register_dl_tips_msg3 || "（邮箱是您接收通知和交易信息的重要渠道，请认真填写。） " }}</span></p>
            <form class="layui-form" action="">
                ...
                <br> <br>
                <p class="reg-title-2">{{ Lang.page_register_dl_tips_msg || "账户信息 " }}<span>{{ Lang.page_register_dl_tips_msg5 || "（填入销售的邀请码，将建立您与他的关联关系；若无，可不填） " }}</span></p>
                <div class="layui-form-item">
    ...
    <div class="register" id="second_page" style="display:none">
        ...
        <div class="register-form">
            <p class="reg-title-2">{{ Lang.page_register_dl_tips_msg || "账户信息 "  }}</p>
            <form class="layui-form" action="">
                ...
                <div class="layui-row" >
                    <div class="layui-col-md5 layui-col-xs12">
                        <div class="layui-form-item">
                            ...
                            <div class="layui-input-block">
                                <select name="account_type" lay-filter="account_type" id="accountType" lay-verify="NotBlank" msg="{{ Lang.validate_account_type_not_chose || "请选择账户类型" }}">
                                   {# {% for at in Lang.select.ib.register.account_type %}
                ...
                <p class="reg-title-2">{{ Lang.page_register_dl_tips_msg  || "证件图片上传 " }}<span>{{ Lang.page_register_dl_tips_msg9 || "(目前只支持png、jpg格式，大小控制在2MB之内) " }}</span></p>
                ...
                <div class="layui-form-item">
                    <p class="layui-input-block" style="margin-left: 0px;">
                        ...
                        <span>{{ Lang.form_text_agree_span_split || " 同意 " }}<a href="/assets/kh/file/cn/BAPPEBTI.pdf" id="agreement" target="_blank" style="color: blue">{{ Lang.form_text_ib_agreement || " 代理协议 " }}</a></span>
                    </p>
                </div>
